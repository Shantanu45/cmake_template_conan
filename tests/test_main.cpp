...
#include <cmake_template_project/sample_library.hpp>

TEST_CASE("configured project metadata is available")
{
  CHECK(cmake_template_project::cmake::project_name.size() > 0);
  CHECK(cmake_template_project::cmake::project_version.size() > 0);
}
