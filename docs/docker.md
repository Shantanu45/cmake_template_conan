...
```bash
docker build -f ./.devcontainer/Dockerfile --tag=cmake_template_project:latest .
docker run -it cmake_template_project:latest
```

This command will put you in a `bash` session in a Ubuntu 20.04 Docker container,
with all of the tools listed in the [dependencies](dependencies.md#dependencies) section already installed.
...
```bash
docker build --tag=cmake_template_project:latest --build-arg GCC_VER=10 --build-arg LLVM_VER=11 .
```
...
```bash
docker build --tag=cmake_template_project:latest --build-arg USE_CLANG=1 .
```

You will be logged in as root, so you will see the `#` symbol as your prompt.
You will be in a directory that contains a copy of `cmake_template_project`;
...
cmake_template_project:latest
```

You can configure and build [as directed above](#build) using these commands:

```bash
/cmake_template_project# mkdir build
/cmake_template_project# cmake -S . -B ./build
/cmake_template_project# cmake --build ./build
```
...
```bash
/cmake_template_project# mkdir build
/cmake_template_project# CC=clang CXX=clang++ cmake -S . -B ./build
/cmake_template_project# cmake --build ./build
```
